...
Drawable::Drawable() {
  mutex.lock();
  drawIndex = gameState.renderer.getNextFreeDrawIndex(this);
}
...
void Drawable::addDependency(Drawable *drawable) {
  ...
  throw std::runtime_error("No free dependency slots left");
}
...
void Drawable::removeDependency(Drawable *drawable) {
  ...
  throw std::runtime_error("Tried to remove non-existent dependency");
}
...
void Drawable::addDependee(Drawable *drawable) {
  ...
  throw std::runtime_error("No free dependee slots left");
}
...
void Drawable::removeDependee(Drawable *drawable) {
  ...
  throw std::runtime_error("Tried to remove non-existent dependee");
}
...
void Drawable::resetMutex() { mutex.lock(); }

std::mutex *Drawable::getMutex() { return &mutex; }
